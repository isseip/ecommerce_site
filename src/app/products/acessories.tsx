...
import products from '../products_Acessories.json';
...
const Products_Acessories = () => {
  const products_acessories: Product[] = products;
        ...
        {products_acessories.map((product) => (
          <div key={product.id} className="rounded-lg bg-base-100 shadow-md overflow-hidden flex flex-col">
            ...
            <div className="p-4 flex flex-col flex-grow text-white"> {/* Changed text color for better readability */}
              ...
              <p className="flex-grow"><Link href={`/Acessories/${product.id}`}>{product.description}</Link></p>
              <p className="text-lg font-semibold mt-4"><Link href={`/Acessories/${product.id}`}>Price: ${product.price}</Link></p>
