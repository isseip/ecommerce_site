...
const Products_Acessories: React.FC = () => {
  const { addToCart } = useCart();
  const { theme } = useTheme();
  const products_acessories: Product[] = products.map((product) => ({
    id: product.id,
    name: product.name,
  ...
  const sortedProducts = [...products_acessories].sort((a, b) => {
  ...
  return (
    <div className="container mx-auto mt-2 p-5">
      ...
      <h2 className={`text-2xl font-bold ${textColor}`}>Acessories Products</h2>
        <div className=''>
