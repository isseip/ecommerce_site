...
const Products_Jewelry: React.FC = () => {
  const { addToCart } = useCart();
  const { theme } = useTheme();
  const products_jewelry: Product[] = products.map((product) => ({
    id: product.id,
    name: product.name,
  ...
  const sortedProducts = [...products_jewelry].sort((a, b) => {
  ...
  return (
    <div className="container mx-auto mt-2 p-5">
      ...
      <h2 className={`text-2xl font-bold ${textColor}`}>Jewelry Products</h2>
        <div>
