...
import products from '@/app/products_Shoes.json';
...
const ProductDetail = () => {
  ...
  return (
    <>
      ...
      <div className="container mx-auto  p-5 bg-base-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {relatedProducts.map((relatedProduct) => (
            <div key={relatedProduct.id} className="rounded-lg bg-base-100 shadow-md overflow-hidden flex flex-col">
              <Link href={`/Shoes/${relatedProduct.id}`}>
                <Image
              ...
              <div className="p-4 flex flex-col flex-grow text-white">
                <h2 className="text-xl font-bold mb-2">
                  <Link href={`/Shoes/${relatedProduct.id}`}>{relatedProduct.name}</Link>
                </h2>
                <p className="flex-grow">
                  <Link href={`/Shoes/${relatedProduct.id}`}>{relatedProduct.description}</Link>
                </p>
                <p className="text-lg font-semibold mt-4">
                  <Link href={`/Shoes/${relatedProduct.id}`}>Price: ${relatedProduct.price}</Link>
                </p>
